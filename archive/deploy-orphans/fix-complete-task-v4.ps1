...
Write-Host "=== Complete Task Fix v4 ===" -ForegroundColor Cyan
...
$hasSavePromise = $script -match 'var savePromise = frm\.is_dirty\(\) \? frm\.save\(\) : Promise\.resolve\(\);'
$hasServerAutoCompleteGuard = $script -match 'Server already completed this task during the first save'
...
Write-Host "New savePromise: $(if($hasSavePromise){'Yes'}else{'No'})"
Write-Host "Server auto-complete guard: $(if($hasServerAutoCompleteGuard){'Yes'}else{'No'})"
Write-Host "Timeout failsafe: $(if($hasTimeout){'Yes'}else{'No'})"
Write-Host "Busy flag: $(if($hasBusy){'Yes'}else{'No'})"

if ($Mode -eq "Check") {
    if (-not $hasOld -and $hasSavePromise -and $hasServerAutoCompleteGuard -and $hasTimeout -and $hasBusy) {
...
if ($start -lt 0 -or $end -lt 0 -or $end -le $start) { throw "Could not locate Complete Task handler" }
...
$backupPath = Join-Path $PSScriptRoot ("_backup_Task_Accept_Start_v4_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".js")
...
Write-Host "Complete Task v4 deployed" -ForegroundColor Green
