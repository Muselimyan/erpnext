frappe.ui.form.on("Task", {
    ...
    refresh(frm) {
        ...
        // Global: fix page title overflow ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ title row 1, buttons row 2
        (function() {
            ...
            if ((!frm.is_new() || isOrderEntry) && frm.doc.status !== "Completed" && frm.doc.status !== "Cancelled") {
                ...
                btn.on("click", function() {
                    ...
                    function restoreOriginalValues() {
                        ...
                        if (frm.doc.status !== originalStatus) {
                            restore = restore.then(function() { return frm.set_value("status", originalStatus); });
                        }
                        if (frm.doc.completed_on !== originalCompletedOn) {
                            restore = restore.then(function() { return frm.set_value("completed_on", originalCompletedOn || ""); });
                        }
                        return restore;
                    }

                    // Strict order: save current edits first, then set completion fields synchronously, then save completion.
                    frm.save()
                        .then(function() {
                            btn.text("Completing...");
                            // Set values synchronously to avoid promise issues
                        ...
                        .catch(function(err) {
                            ...
                            restoreOriginalValues().then(resetButton).catch(resetButton);
                        });
                });
