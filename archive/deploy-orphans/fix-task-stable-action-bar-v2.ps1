...
Write-Host "=== Fix Task Stable Action Bar v2 ===" -ForegroundColor Cyan
...
$script = $data.script
$hasOldSubject = $script -match 'Subject:'
$hasBar = $script -match 'renderStableTaskActionBar'
$hasV2 = $script -match 'stable-task-action-bar-v2'

Write-Host "Has stable action bar: $(if($hasBar){'Yes'}else{'No'})"
Write-Host "Has duplicate subject in bar: $(if($hasOldSubject){'Yes'}else{'No'})"
Write-Host "Has v2 marker: $(if($hasV2){'Yes'}else{'No'})"

if ($Mode -eq "Check") {
    if ($hasBar -and -not $hasOldSubject -and $hasV2) { Write-Host "Status: fixed" -ForegroundColor Green } else { Write-Host "Status: needs deploy" -ForegroundColor Yellow }
    return
}

$pattern = '(?s)function renderStableTaskActionBar\(frm\) \{.*?\n\}'
$newFunction = @'
...
if ($script -notmatch $pattern) { throw "Could not find renderStableTaskActionBar function" }
$script = [regex]::Replace($script, $pattern, $newFunction, 1)

$backupPath = Join-Path $PSScriptRoot ("_backup_Task_Accept_Start_stable_bar_v2_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".js")
...
Write-Host "Stable Task action bar v2 deployed" -ForegroundColor Green
