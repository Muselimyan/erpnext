...
## Doc 08 — Reorder & Ordering by Supplier

### Completed (2026-05-08)

**Role**:
- `Ops - Purchasing Lead` — created with `desk_access = 1`.

**Custom Field — Item**:
- `reorder_change_reason` — Small Text; inserted after `reorder_levels`; not required, print-hidden. Governance-enforced by server script below.

**Server Script**:
- `Item-before-save-reorder-change-reason` — Item / Before Save: if `reorder_levels` rows change (warehouse, reorder_level, or reorder_qty), hard-blocks save unless `reorder_change_reason` is non-blank (Doc 08 governance rule).

**Role Permission — Item**:
- `Ops - Purchasing Lead`: Read = ON, Write = ON (Custom DocPerm record `j08rtbhkr0`). Allows purchasing leads to set/edit reorder thresholds on Items.

### Pending / To-Do
- **[ ]** Set actual reorder threshold values per item (`Item.reorder_levels` child table, warehouse = `Main - Inmed`) — buyer team data-entry action; no schema change needed.
- **[ ]** Assign real staff user(s) to `Ops - Purchasing Lead` role once identified.
- **[ ]** Close Doc 06 §D: broader Item permission restructuring (`Ops - Inventory` Write=ON, others Write=OFF) is still pending and intentionally deferred; `Ops - Purchasing Lead` permission row above is the only Doc 08 addition.
- **[ ]** Re-export deploy snapshot after completing item threshold data entry.

### Notes / Known Issues
- `Main - WH` in Doc 08 documentation maps to `Main - Inmed` in production — server script uses the production warehouse name.
- Frappe on this instance rejects `return` at the top level of a Server Script (SyntaxError). Script was written using `if before:` nesting instead of early return.
- No custom saved reports created — ERPNext's built-in **Stock Reorder** page (`/stock-reorder`) covers the "always-available reorder list" requirement from Doc 08A §5.3. Saved custom reports can be added later if buyer team requests a grouped-by-supplier view.
- Item reorder thresholds were empty at deployment time (0 items had `reorder_levels` rows set).
