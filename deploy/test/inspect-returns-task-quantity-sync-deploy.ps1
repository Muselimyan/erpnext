...
$ServerName = "task_update_return_item_quantities"
$ExistingServer = Get-ErpDoc "Server Script" $ServerName
if ($Mode -eq "Deploy") {
    $Report.server_scripts += Upsert-ErpDoc "Server Script" $ServerName ([ordered]@{
        script_type = "API"
        api_method = "task_update_return_item_quantities"
        allow_guest = 0
        disabled = 0
        enable_rate_limit = 0
        script = $ReturnUpdateApi
    })
} else {
    $Report.server_scripts += [pscustomobject]@{ name = $ServerName; exists = ($null -ne $ExistingServer); enabled = if ($ExistingServer) { 1 - [int]$ExistingServer.disabled } else { $null } }
}

$ClientName = "Task-Packing Checkboxes"
$ExistingClient = Get-ErpDoc "Client Script" $ClientName
if ($Mode -eq "Deploy") {
    if ($ExistingClient) {
        $BackupPath = Join-Path $PSScriptRoot ("_backup_Task_Packing_Checkboxes_before_return_qty_sync_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".js")
        Set-Content -Path $BackupPath -Value ([string]$ExistingClient.script) -Encoding UTF8
        $Report.notes += "Backup: $BackupPath"
    }
    ...
    $hasReturnApiCall = $ExistingClient -and ([string]$ExistingClient.script) -match 'task_update_return_item_quantities'
    $hasRestockingFilter = $ExistingClient -and ([string]$ExistingClient.script) -match 'task_product_work_area_render_restocking'
    $hasInvoiceFilter = $ExistingClient -and ([string]$ExistingClient.script) -match 'task_product_work_area_render_invoice_preparation'
    $Report.client_scripts += [pscustomobject]@{ name = $ClientName; exists = ($null -ne $ExistingClient); has_return_qty_sync = $hasReturnApiCall; has_restocking_filter = $hasRestockingFilter; has_invoice_filter = $hasInvoiceFilter }
}

$Report.notes += "Inspect Returns Task form will edit returned/lost quantities while Dispatch Case remains source of truth."
$Report.notes += "Returns restocking Task form displays only Dispatch Case rows with returned_qty > 0."
$Report.notes += "Invoice preparation Task form displays only Dispatch Case rows with used_qty > 0 or lost_damaged_qty > 0."
$Report.notes += "Phone layout defaults to compact 4-column table and has remembered toggle for detailed cards; laptop layout keeps the full table."
$Report.notes += "Packing task checkbox behavior is preserved through task_mark_item_packed."
