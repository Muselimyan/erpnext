...
def main():
    print(f"Rolling back forced Subject visibility on TEST only: {BASE_URL}")
    doc = request("GET", f"/api/resource/{enc('Client Script')}/{enc(SCRIPT_NAME)}")["data"]
    script = doc.get("script") or ""

    forced_block = '''        var subjectControl = wrapper.find('[data-fieldname="subject"]').closest('.frappe-control');
    ...
    original_line = '''        var photosControl = wrapper.find('[data-fieldname="custom_account_photos"]').closest('.frappe-control');'''
    script = script.replace(forced_block, original_line, 1)

    forced_insert = '''        if (subjectControl.length) {
            photosBoxHost.insertAfter(subjectControl);
        } else if (statusControl.length) {
            photosBoxHost.insertBefore(statusControl);'''
    original_insert = '''        if (statusControl.length) {
            photosBoxHost.insertBefore(statusControl);'''
    script = script.replace(forced_insert, original_insert, 1)
    ...
    print("Rollback complete on TEST. Only the last forced Subject visibility change was reverted.")
