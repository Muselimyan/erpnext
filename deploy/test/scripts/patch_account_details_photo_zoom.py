...
def main():
    print(f"Patching Account Details image zoom on TEST only: {BASE_URL}")
    ...
    old = '''            var dialog = new frappe.ui.Dialog({ title: title, size: 'large' });
            ...
            dialog.show();'''
    new = '''            var dialog = new frappe.ui.Dialog({ title: title, size: 'large' });
    ...
    if old not in script and new not in script:
        raise RuntimeError("Expected preview dialog block not found")
    ...
    print("Patch complete on TEST. Image preview now has zoom controls.")
