...
$ScriptName = "Task-Mobile Form Layout Fix"
$NewScript = @'
...
Write-Host "=== Task mobile form layout fix ===" -ForegroundColor Cyan
...
if ($existing) {
    $backupPath = Join-Path $PSScriptRoot ("_backup_Task_Mobile_Form_Layout_Fix_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".js")
...
Write-Host "Done. Hard-refresh the phone browser and reopen the Task form." -ForegroundColor Green
