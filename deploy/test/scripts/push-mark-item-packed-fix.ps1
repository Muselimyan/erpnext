...
# Quick push: task_mark_item_packed.py fix (packed string truthy bug)
Set-StrictMode -Off
...
$FilePath = Join-Path $WorkDir "server\task_mark_item_packed.py"
...
$Json = @{ script = $Content } | ConvertTo-Json -Depth 5 -Compress
Invoke-RestMethod -Uri "$BaseUrl/api/resource/Server%20Script/task_mark_item_packed" -Headers $Headers -Method Put -Body ([System.Text.Encoding]::UTF8.GetBytes($Json)) -TimeoutSec 60 | Out-Null
Write-Host "task_mark_item_packed: UPDATED" -ForegroundColor Green
