...
$ScriptName = "Telegram Task Assignment Notification"
$NewScript = @'
...
Write-Host "=== Patch Telegram Task assignment targeting ===" -ForegroundColor Cyan
...
$backupPath = Join-Path $PSScriptRoot ("_backup_Telegram_Task_Assignment_Notification_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".py")
$existing.data.script | Set-Content -Path $backupPath -Encoding UTF8

$body = @{ script = $NewScript; disabled = 0 } | ConvertTo-Json -Depth 10 -Compress
...
Write-Host "Direct user assignment now notifies only that user; team placeholders expand by role." -ForegroundColor Green
