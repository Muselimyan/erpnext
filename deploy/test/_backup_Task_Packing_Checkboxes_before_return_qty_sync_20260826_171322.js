...
function task_product_work_area_render_returns(frm, doc, rows, show_alert) {
    ...
    html += `<style>
        .task-return-mobile-list { display: none; }
        @media (max-width: 767px) {
            .task-return-desktop-table { display: none; }
            .task-return-mobile-list { display: block; }
    ...
    let mobile_html = `<div class="task-return-mobile-list">`;
    rows.forEach(function(row, idx) {
        ...
        const mobile_checkbox_id = `return_mobile_checkbox_${idx}`;
        ...
        mobile_html += `<div class="task-return-card" data-return-row="${idx}">
            <div class="task-return-card-title">${item_label}</div>
            <div class="task-return-card-grid">
                <div class="task-return-card-full" data-dispatched="${dispatched}"><label><input type="checkbox" id="${mobile_checkbox_id}" data-idx="${idx}" ${checked ? 'checked' : ''} onchange="task_product_work_area_toggle_returned(this, '${escaped_case}', ${idx})"> Returned?</label></div>
