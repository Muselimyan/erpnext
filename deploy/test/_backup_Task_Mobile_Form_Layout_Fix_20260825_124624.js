...
function task_mobile_pack_photo_button(frm) {
    ...
    if (String(frm.doc.task_kind || '').trim() !== 'Pack / prepare items') return;
    var $w = $(frm.wrapper);
    var $btn = $w.find('#task-mobile-pack-add-photos-btn');
    if (!$btn.length) {
        $btn = $('<button id="task-mobile-pack-add-photos-btn" class="btn btn-sm btn-primary" type="button" style="font-size:12px;padding:6px 16px;background:#000;border-color:#000;color:#fff;border-radius:7px;margin-top:8px;margin-bottom:12px;display:inline-block;">+ Add Photos</button>');
                    ...
                    callback: function(r) {
                        ...
                        new frappe.ui.FileUploader({
                            ...
                            on_success: function(file_doc) {
                                ...
                                task_mobile_pack_set_pickup_photo(frm, url, true);
                                task_mobile_pack_photo_preview(frm);
                                setTimeout(function() { task_mobile_pack_photo_preview(frm); }, 800);
                            }
                        });
                    }
                });
    ...
    var $anchor = $w.find('[data-fieldname="warehouse_pickup_photo"]').closest('.frappe-control').first();
    if (!$anchor.length) {
        $anchor = $w.find('.control-label, label').filter(function() { return $.trim($(this).text()) === 'Warehouse Pickup Photo'; }).closest('.frappe-control').first();
    }
    if ($anchor.length) {
        $btn.detach().insertAfter($anchor.find('.control-input-wrapper, .control-value').last());
        task_mobile_pack_photo_preview(frm);
        setTimeout(function() { task_mobile_pack_photo_preview(frm); }, 900);
    }
}

function task_mobile_pack_photo_preview(frm) {
    if (!frm || !frm.doc || !frm.doc.name || !frm.wrapper || frm.is_new()) return;
    if (String(frm.doc.task_kind || '').trim() !== 'Pack / prepare items') return;
    var $w = $(frm.wrapper);
    var $btn = $w.find('#task-mobile-pack-add-photos-btn');
    if (!$btn.length) return;
    var $host = $w.find('#task-mobile-pack-photo-preview');
    if (!$host.length) {
        $host = $('<div id="task-mobile-pack-photo-preview" style="margin-top:8px;margin-bottom:12px;"></div>');
        $btn.after($host);
    }
    ...
    function render(files) {
        ...
        task_mobile_pack_set_pickup_photo(frm, images[0].file_url, false);
...
function task_mobile_pack_set_pickup_photo(frm, url, save_now) {
    if (!frm || !frm.doc || !url) return;
    if (String(frm.doc.task_kind || '').trim() !== 'Pack / prepare items') return;
    if (frm.doc.warehouse_pickup_photo) return;
    if (frm.fields_dict && frm.fields_dict.warehouse_pickup_photo) {
        frm.set_value('warehouse_pickup_photo', url);
        ...
        frappe.call({
            ...
            callback: function() {
                frm.doc.warehouse_pickup_photo = url;
                task_mobile_pack_photo_preview(frm);
            }
        });
    }
