...
However, this audit identified **5 original findings** (1 fully fixed, 2 reclassified as not-bugs, 2 partially fixed), **7 documentation gaps**, **4 legacy/dead-code risks**, and **3 design concerns**. Summary of bug resolutions:
...
| BUG-04 | ~~"Invoiced" state never set by any script~~ | ~~Medium~~ **PARTIALLY FIXED — docs updated; live DocType status options still include `Invoiced`** | 0.95 |
| BUG-05 | ~~`Task-Product Lines Display.js` sets wrong default warehouse~~ | ~~Medium~~ **PARTIALLY FIXED — local file disabled; live schema still enabled: 1** | 0.95 |
...
| GAP-02 | Missing role restriction for `Return Call` task (`Returns restocking` already mapped) | Low | 0.90 |
...
**Severity: MEDIUM | Confidence: 0.95 | Status: PARTIALLY FIXED — documentation and local scripts updated; live `Dispatch Case` DocType status options still include `Invoiced` and must be removed during next deployment**
...
**Severity: MEDIUM | Confidence: 0.95 | Status: PARTIALLY FIXED — disabled in local work file; live test `client-scripts.json` still shows `enabled: 1`, so deployment to disable it is still required**
...
### GAP-02: Missing Role Restriction for `Return Call` Task
**Severity: LOW | Confidence: 0.90**

**Evidence:** `dispatch_task_accept.py` lines 16–29

**Problem:** The `TASK_KIND_ALLOWED_ROLES` map does not include an entry for `Return Call`. `Returns restocking` is already mapped to `Ops - Returns` in the live script (re-check corrected).

When `allowed` is empty for `Return Call` (line 31), the role check is bypassed (line 38: `if allowed and not has_allowed_role`). Anyone with any role can accept a `Return Call` task.
...
> **Status update (2026-08-28):** Local `Dispatch-Case-after-save.py` has been updated to use the same dynamic director lookup. Deployment to test/prod is still required.
...
3. ~~**Fix BUG-05**~~: **PARTIALLY FIXED** — `Task-Product Lines Display.js` is disabled in the local work file, but live test `client-scripts.json` still shows `enabled: 1`. Deploy the disabled script and promote `Task-Create Dispatch Case Items.js`.
...
5. ~~**Fix BUG-04**~~: **PARTIALLY FIXED** — `Invoiced` state removed from all current docs (Doc 16 §6.9/§10, Doc 16A §7/§9.3, both walkthroughs). Still need to remove `Invoiced` from the live `Dispatch Case` DocType status field options before next deployment.
...
8. **Add missing role restrictions** (GAP-02): `Return Call` entry added to `TASK_KIND_ALLOWED_ROLES` in local `dispatch_task_accept.py`; `Returns restocking` was already mapped. Deploy the updated script to test.
