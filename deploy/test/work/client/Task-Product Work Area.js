...
function task_product_work_area_refresh(frm, show_alert) {
    ...
    frappe.call({
        ...
        callback: function(r) {
            ...
            let html = `<div class="small text-muted" style="margin-bottom:8px">Source: <b>${frappe.utils.escape_html(doc.name)}</b> ÃÂÃÂ· Customer: <b>${frappe.utils.escape_html(doc.customer || "")}</b></div>`;
