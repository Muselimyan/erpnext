...
|---|---------|----------|------------|
| B-01 | Payment Entry created without invoice references — **DEPLOYED TO TEST** in extracted script: allocations are preserved and written as Sales Invoice references on the Payment Entry | **CRITICAL — deployed to TEST, check-mode verified; functional TEST smoke test passed; production/main not touched** | High (code-proven) |
...
| B-03 | Advance payment `prepaid_amount` overwrites instead of accumulating on Dispatch Case — **DEPLOYED TO TEST** with Dispatch Case advance-payment child-table audit trail and recalculated prepaid total | **HIGH — deployed to TEST, schema/check-mode verified; functional TEST smoke test passed; production/main not touched** | High (code-proven) |
| B-04 | Tender Agreement `supplied_quantity` deducted from ALL matching tenders for same customer — **DEPLOYED TO TEST**: one active tender per hospital/item is enforced and over-supply stops invoice submit with review message | **HIGH — deployed to TEST, check-mode verified; functional TEST tender smoke test passed; production/main not touched** | High (code-proven) |
| B-05 | Tender quantities were never reversed on invoice cancellation — **DEPLOYED TO TEST** with Sales Invoice tender fulfillment audit trail and After Cancel reversal script | **HIGH — deployed to TEST, schema/check-mode verified; functional TEST cancellation smoke test passed; production/main not touched** | High (code-proven) |
| B-06 | Tender `Closed` status was overridden by auto-status logic on save — **DEPLOYED TO TEST** so manual `Closed` status is preserved | **MEDIUM — deployed to TEST, check-mode verified; pending functional tender status smoke test/prod promotion** | High (code-proven) |
| B-07 | Debt Closure profit calculation only considered one invoice — **DEPLOYED TO TEST** to calculate total profit across all linked Open Invoices and write per-Dispatch Case profit | **MEDIUM — deployed to TEST, check-mode verified; functional TEST debt-closure smoke test fully passed; production/main not touched** | High (code-proven) |
| B-08 | Scheduler and dispatch flow both created/updated `Debt Collection` with different assignees/data — **DEPLOYED TO TEST** by separating scheduler alerts into `Debt Alert` | **MEDIUM — deployed to TEST, schema/check-mode verified; functional TEST scheduler/debt-flow smoke test passed; production/main not touched** | High (code-proven) |
| B-09 | Hardcoded user emails in debt closure approval — **DEPLOYED TO TEST** to use `Debt Closure Approval` Task Access Policy for assignment and completion permission | **LOW — deployed to TEST, policy/check-mode verified; functional TEST approval-policy smoke test fully passed; production/main not touched** | High (code-proven) |
| B-10 | Financial field visibility was client-side only — **DEPLOYED TO TEST** with permlevel/DocPerm deployment helper and client script alignment: payment/profit fields restricted to financial roles; pricing fields also allowed for Order Creating | **MEDIUM — deployed to TEST, schema/check-mode verified; functional TEST role-permission smoke test fully passed; production/main not touched** | High (code-proven) |
| B-11 | FIFO payment allocation sorted by invoice name, not invoice date — **DEPLOYED TO TEST** to sort by Sales Invoice posting date with invoice name tie-breaker | **LOW — deployed to TEST, check-mode verified; functional TEST smoke test passed; production/main not touched** | High (code-proven) |
| B-12 | `paid_to` account hardcoded as "Cash - Inmed" regardless of payment method — **DEPLOYED TO TEST** with Cash/Bank Transfer/Card account mapping and TEST `Bank - Inmed` leaf account | **MEDIUM — deployed to TEST, account/check-mode verified; functional TEST cash/bank account smoke test passed; production/main not touched** | High (code-proven) |
| B-13 | `frappe.db.commit()` inside loop in tender update script breaks transaction boundary — **DEPLOYED TO TEST** by removing manual commits and leaving tender updates inside Sales Invoice submit transaction | **MEDIUM — deployed to TEST, check-mode verified; functional TEST tender smoke test passed; production/main not touched** | High (code-proven) |
...
| 2026-09-03 | B-02/D-04 | Confirmed the unused `Payment Entry-after-submit-distribute-payment` Server Script is already disabled in local metadata/schema. Updated central/manual/implementation documentation so Distribute Payment is shown as disabled/deferred pending final keep/delete decision with the colleague. | Documentation decision recorded; script remains disabled, not deleted |
| 2026-09-02 | B-03 | `Task-after-save-advance-payment.py` now appends each linked-case advance to a Dispatch Case `advance_payments` child table, recalculates `prepaid_amount` from all child rows, and keeps `prepaid_payment_entry` as latest-entry quick reference. Added TEST deployment script to create the child table/custom field and update the server script. Doc 16 and the customer advance payment manual updated to describe the audit trail and accumulated total. | Deployed to TEST on 2026-09-04; schema/check-mode verified; functional TEST accounting smoke test passed on 2026-09-07; production/main not touched |
| 2026-09-02 | B-04, B-13 | `Sales-Invoice-after-submit-tender-update.py` now updates at most one active tender row per hospital/item, stops submit when duplicate active tender rows exist, stops submit on over-supply for Accounting/Director review, and removes manual `frappe.db.commit()`. Added TEST deployment script and tender management manual. | Deployed to TEST on 2026-09-04; check-mode verified; functional TEST tender submit smoke test passed on 2026-09-08; over-supply blocking behavior verified; production/main not touched |
| 2026-09-03 | Tender price/status | Added `Sales-Invoice-before-submit-tender-validation.py` to block tender invoice submit when the invoice rate does not match `Tender Agreement Item.tender_price`. Confirmed business rule: tenders are hospital-only, so matching remains `Sales Invoice.customer == Tender Agreement.hospital`. Updated `Tender-Agreement-before-save.py` so only `Closed` is preserved; `Draft` now auto-transitions by valid dates. | Deployed to TEST on 2026-09-04; check-mode verified; functional TEST tender price/status smoke tests passed on 2026-09-08; production/main not touched |
| 2026-09-03 | D-03 | Completed Tender Agreement documentation coverage with `docs/manual/tender-agreement-management.md` and index/overview links. Manual covers create/update workflow, duplicate active tender handling, over-supply blocking, cancellation reversal from tender fulfillment audit rows, Closed status preservation, date-based Draft/Active/Expired status, hospital-only matching, tender price enforcement, and transaction safety. | Documentation updated; related tender behavior deployed to TEST on 2026-09-04 and functional TEST tender smoke tests passed on 2026-09-08 |
| 2026-09-02 | B-05 | Tender fulfillment now writes a Sales Invoice `tender_fulfillments` child-table audit trail on submit. New `Sales-Invoice-on-cancel-tender-reversal.py` reverses exactly those recorded tender quantities on invoice cancellation. Added TEST deployment script for the child DocType/custom field and cancel script. | Deployed to TEST on 2026-09-04; schema/check-mode verified; functional TEST cancellation smoke test passed on 2026-09-08 after event correction to After Cancel; production/main not touched |
| 2026-09-02 | B-06 | `Tender-Agreement-before-save.py` now preserves manually Closed tenders by skipping date-based auto-status when `status == "Closed"`. Tender manual updated to document that Closed stays Closed. | Deployed to TEST on 2026-09-04; check-mode verified; functional TEST tender status smoke test passed on 2026-09-08; production/main not touched |
| 2026-09-03 | B-07 | `Task-after-save-debt-closure.py` now calculates Debt Closure Approval profit across all unique Sales Invoices in `open_invoices`, sums total profit into `custom_case_profit`, and writes each linked Dispatch Case's own profit. Added TEST deployment script and linked Debt Closure Approval manual. | Deployed to TEST on 2026-09-04; check-mode verified; functional TEST multi-invoice and per-Dispatch Case debt-closure smoke test fully passed on 2026-09-08; production/main not touched |
| 2026-09-03 | B-08 | `Scheduled-debt-collection.py` now creates/updates Director `Debt Alert` tasks instead of Finance `Debt Collection` tasks. Dispatch flow remains responsible for `Debt Collection` with Open Invoices/payment recording. Added TEST deployment script to add the `Debt Alert` task kind/policy and updated Doc 16 plus debt alert/manual docs. | Deployed to TEST on 2026-09-04; schema/check-mode verified; functional TEST scheduler/debt-flow smoke test passed on 2026-09-08; production/main not touched |
| 2026-09-03 | D-02 | Completed documentation cleanup for scheduled GL-based debt threshold alerts: requirements, docs overview, migration notes, Doc 16, and debt alert manual now consistently describe Director `Debt Alert` tasks rather than Finance `Debt Collection` tasks for threshold alerts. | Documentation updated; related B-08 behavior deployed to TEST on 2026-09-04 and functional TEST scheduler/debt-flow smoke test passed on 2026-09-08 |
| 2026-09-03 | B-09 | `Task-after-save-debt-closure.py` and `Task-before-save-dispatch-gates.py` now use the `Debt Closure Approval` Task Access Policy instead of hardcoded approver emails. Approval task assignment uses the policy default team user; completion permission uses the policy allowed roles. Added TEST deployment script and updated Debt Closure Approval documentation. | Deployed to TEST on 2026-09-04; policy/check-mode verified; functional TEST approval-policy smoke test fully passed on 2026-09-08; production/main not touched |
| 2026-09-03 | B-10 | Added TEST deployment helper for server-side field protection: Dispatch Case payment/profit fields move to permlevel 1 for Accounting/Finance/Directors/System Manager; Dispatch Case Item price/discount fields move to permlevel 2 for Order Creating plus financial roles. Updated client script to match and hide `profit`/`advance_payments` from non-financial users. | Deployed to TEST on 2026-09-04; schema/DocPerm/check-mode verified; functional TEST role-permission smoke test fully passed on 2026-09-08; production/main not touched |
| 2026-09-03 | B-12 | `Task-before-save-payment-recording.py` and `Task-after-save-advance-payment.py` now map `paid_to` by payment method: Cash → `Cash - Inmed`, Bank Transfer/Card → `Bank - Inmed`. Added TEST deployment script with account creation/checks and updated payment manuals/Doc 16. | Deployed to TEST on 2026-09-04; created TEST `Bank - Inmed` leaf account; account/check-mode verified; functional TEST cash/bank Payment Entry smoke tests passed on 2026-09-07; production/main not touched |
...
| 2026-09-04 | TEST deployment | Deployed Group 3 B-01, B-03, B-04, B-05, B-06, B-07, B-08, B-09, B-10, B-11, B-12, and B-13 to `https://test.erpnext.am`. Created TEST `Bank - Inmed` leaf account for B-12. Cleared TEST cache and exported TEST schema/data snapshot. Added TEST-only smoke-test manual at `deploy/test/manual/group-3-smoke-test.md`. | TEST deployment/check-mode verification complete; functional user smoke tests all passed on TEST; production/main intentionally not touched |
| 2026-09-07 | B-01/B-11 RestrictedPython fix | First smoke test discovered `NameError: name 'invoice_dates' is not defined` in `Task-before-save-payment-recording.py`. Root cause: RestrictedPython does not allow `lambda` closures to reliably access outer-scope variables. Fixed by replacing `invoice_dates` dict + `sorted(... lambda ...)` pattern with a plain list of sortable tuples `(posting_date, invoice_name, row)`. Also replaced generator-based `sum(... for ...)` with plain loop for total outstanding. Follow-up fixed payment-history linking by setting the appended child row's `payment_entry` directly after Payment Entry submit. Redeployed patched script to TEST and cleared cache. FIFO logic and allocation behavior unchanged; only RestrictedPython-safe implementation. | Deployed to TEST on 2026-09-07; functional cash and Bank Transfer smoke tests passed for Payment Entry creation/submission, invoice references, FIFO allocation, Cash/Bank account mapping, task outstanding update, final completion, and Payment History links. |
| 2026-09-07 | B-03/B-12 Section 2 Customer Advance smoke test | Before Customer Advance smoke testing, `Task-after-save-advance-payment.py` was patched for RestrictedPython safety: `reference_date` now uses `frappe.utils.today()` explicitly, and `prepaid_amount` recalculation uses a plain loop instead of generator-based `sum(... for ...)`. User completed `TASK-2026-00543` for Cash `10,000` and `TASK-2026-00544` for Bank Transfer `20,000`, both linked to Dispatch Case `DC-2026-00144`. Payment Entries `ACC-PAY-2026-00009` and `ACC-PAY-2026-00010` were created as drafts with no invoice references; Cash posted to `Cash - Inmed`, Bank Transfer posted to `Bank - Inmed`. Dispatch Case advance rows accumulated correctly and `prepaid_amount = 30,000`, latest `prepaid_payment_entry = ACC-PAY-2026-00010`. | Deployed/tested on TEST on 2026-09-07; Section 2 functional smoke test passed. |
| 2026-09-08 | B-04/B-05/B-13 Section 3 Tender submit smoke test | Prepared customer `G3-SMOKE-HOSPITAL-001 - Inmed`, Tender Agreement `G3 Section 3 Smoke Tender 2026-09-08`, and Sales Invoices `ACC-SINV-2026-00016` through `ACC-SINV-2026-00019`. Valid-price invoice submitted and increased supplied quantity to `2` with remaining `8`, and wrote a Sales Invoice `tender_fulfillments` row. Wrong-price invoice was blocked and stayed Draft with no tender quantity change. Oversupply invoice was blocked and stayed Draft with no tender quantity change. Duplicate-active-tender invoice was blocked and stayed Draft; duplicate tender was closed after verification. | Tested on TEST on 2026-09-08; Section 3 functional smoke test passed. |
| 2026-09-08 | B-05 Section 4 Tender cancellation smoke test | First cancellation of `ACC-SINV-2026-00016` exposed that `Sales-Invoice-on-cancel-tender-reversal.py` was configured with invalid Server Script event `On Cancel`, so the invoice cancelled but tender quantity did not reverse. Fixed event to `After Cancel` and kept RestrictedPython-safe non-negative reversal logic. Prepared/submitted retest invoice `ACC-SINV-2026-00020`; user cancelled it; tender reversed exactly by recorded fulfillment quantity `2`, from supplied `4`/remaining `6` to supplied `2`/remaining `8`. | Fixed/tested on TEST on 2026-09-08; Section 4 functional smoke test passed. |
| 2026-09-08 | B-06 Section 5 Tender status smoke test | Prepared `G3 Section 5 Closed Status Tender 2026-09-08` and `G3 Section 5 Date Flow Tender 2026-09-08`. Closed tender stayed `Closed` after save/date changes. Date-flow tender stayed `Draft` for future dates, became `Active` for current active dates, and became `Expired` for past dates. Remaining quantity recalculation stayed correct. | Tested on TEST on 2026-09-08; Section 5 functional smoke test passed. |
| 2026-09-08 | B-07/B-09 Section 6 Debt Closure Approval smoke test | Patched `Task-after-save-debt-closure.py` for RestrictedPython-safe explicit loops and fixed After Save persistence by writing `custom_case_profit` with `frappe.db.set_value`. Initial `TASK-2026-00542` verified policy assignment/completion but exposed non-persisted profit. Retest task `TASK-2026-00546` was accepted/completed by `levonaghinyan77@gmail.com`; `custom_case_profit` persisted as `6,239,443`, matching independent calculation across `ACC-SINV-2026-00008` and `ACC-SINV-2026-00015`. Added missing `Debt Collection Invoice.dispatch_case` child field on TEST and prepared full retest `TASK-2026-00548`; completion persisted `custom_case_profit = 6,239,443` and wrote per-case profits `DC-2026-00150 = 4,680,000`, `DC-2026-00151 = 1,559,443`. | Fixed/tested on TEST on 2026-09-08; Section 6 fully passed. Completion warning about missing Standard Buying prices for BO.102.* items is expected under current implementation and was recorded. |
| 2026-09-08 | B-08 Section 7 Scheduled Debt Alert smoke test | Verified `Scheduled-debt-collection` is enabled as an Hourly Scheduler Event, `Task-task_kind` includes `Debt Alert`, and `Task Access Policy` `Debt Alert` assigns to `directors.team@example.com` with `Ops - Directors`. First manual scheduler run exposed the Task acceptance gate blocking post-insert scheduler updates; patched `Scheduled-debt-collection.py` so new tasks set debt/threshold/description before insert and existing tasks update those fields with `frappe.db.set_value`. Manual TEST scheduler run created `TASK-2026-00549`, `TASK-2026-00550`, and `TASK-2026-00551` as `Debt Alert` tasks with current debt and threshold values. User verified `TASK-2026-00549` in UI and backend confirmed only the Debt Alert exists for that customer, with ToDo assigned to `directors.team@example.com`; no scheduler-created `Debt Collection` task was found. | Fixed/tested on TEST on 2026-09-08; Section 7 passed. |
| 2026-09-08 | B-10 Section 8 Financial field permission smoke test | Used Dispatch Case `DC-2026-00144` for real-user UI testing and backend permission checks. Backend verified financial fields (`sales_invoice`, `prepaid_amount`, `prepaid_payment_entry`, `total_invoice_amount`, `total_paid_amount`, `outstanding_amount`, `profit`, `advance_payments`) are permlevel 1 and restricted to Accounting/Finance/Directors/System Manager, while pricing fields (`unit_price`, `discount_pct`) are permlevel 2 and available to Order Creating plus financial/director roles. Real-user UI checks passed: `driver.01@example.com` could not see financial or pricing fields; `artursemerjyan91@gmail.com` could see Unit Price/Discount % in Case Items row detail while financial fields stayed hidden; Finance/Accounting and Directors users could see financial fields, Advance Payments, Unit Price, and Discount %. Live `Dispatch Case-Price Visibility` client script was enabled. | Tested on TEST on 2026-09-08; Section 8 fully passed. |
...
| S1-F02 | **LOCALLY CORRECTED:** Assignment now comes from the `Debt Alert` Task Access Policy `default_team_user` instead of the first director alphabetically. | B-08 deployed to TEST, pending functional smoke testing/prod promotion | High |
| S1-F03 | **LOCALLY CORRECTED:** Scheduler no longer creates incomplete Finance Debt Collection tasks. It creates Director Debt Alert tasks, which intentionally do not contain `open_invoices`. | B-08 deployed to TEST, pending functional smoke testing/prod promotion | High |
| S1-F04 | **LOCALLY CORRECTED:** Scheduler and dispatch flow now use separate task kinds: `Debt Alert` for Director threshold visibility and `Debt Collection` for Finance payment workflow. | B-08 deployed to TEST, pending functional smoke testing/prod promotion | High |
...
| S1-F06 | **LOCALLY CORRECTED:** Scheduler subject now uses `Debt Alert - {customer_name}`, clearly separate from dispatch-flow `Debt Collection: {customer_name}`. | B-08 deployed to TEST, pending functional smoke testing/prod promotion | High |
...
| ID | Finding | Classification | Confidence |
|----|---------|---------------|------------|
| S2-F01 | **B-01** (above) — PE previously had no invoice references; locally corrected to require linked Sales Invoices for payable rows, preserve allocations, and append Sales Invoice references to the Payment Entry. | **DEPLOYED TO TEST — pending functional smoke testing/prod promotion** | High |
| S2-F02 | **B-11** — FIFO previously sorted by `sales_invoice` name. Locally corrected to sort by linked Sales Invoice posting date, with invoice name as a tie-breaker. | **DEPLOYED TO TEST — pending functional smoke testing/prod promotion** | High |
...
| S2-F04 | **LOCALLY CORRECTED:** `paid_to` now maps by payment method: Cash → `Cash - Inmed`, Bank Transfer/Card → `Bank - Inmed`. | **B-12 deployed to TEST, pending functional smoke testing/prod promotion** | High |
...
| ID | Finding | Classification | Confidence |
|----|---------|---------------|------------|
| S3-F01 | **LOCALLY CORRECTED:** Debt Closure Approval assignment and completion permission now read from the `Debt Closure Approval` Task Access Policy instead of hardcoded email addresses. | **B-09 deployed to TEST, pending functional smoke testing/prod promotion** | High |
| S3-F02 | **LOCALLY CORRECTED:** Replaced stale named-user error text with a policy-based error message. | **B-09 deployed to TEST, pending functional smoke testing/prod promotion** | High |
| S3-F03 | **LOCALLY CORRECTED:** Profit now uses all unique Sales Invoices in `open_invoices`, with a fallback to `doc.sales_invoice` only when no invoice rows exist. | **B-07 deployed to TEST, pending functional smoke testing/prod promotion** | High |
...
| S3-F05 | **LOCALLY CORRECTED:** Dispatch Case profit is written per linked Dispatch Case from that Dispatch Case's own invoice profit, instead of only the first/singular case. | **B-07 deployed to TEST, pending functional smoke testing/prod promotion** | High |
...
| S3-F07 | **LOCALLY CORRECTED:** Debt Closure Approval assignment now goes to the Task Access Policy default team user instead of the first two hardcoded users. | **B-09 deployed to TEST, pending functional smoke testing/prod promotion** | High |
...
| S4-F02 | **LOCALLY CORRECTED:** `prepaid_amount` is recalculated from all Dispatch Case `advance_payments` child rows, so multiple partial advances accumulate correctly. | **B-03 deployed to TEST, pending functional smoke testing/prod promotion** | High |
| S4-F03 | **LOCALLY CORRECTED:** every linked-case advance now has its own child-row audit trail with amount, method, reference, Payment Entry, and source task. `prepaid_payment_entry` remains latest-entry quick reference only. | **B-03 deployed to TEST, pending functional smoke testing/prod promotion** | High |
...
| S4-F05 | **LOCALLY CORRECTED:** advance Payment Entry `paid_to` now uses the same method mapping as Debt Collection payments: Cash → `Cash - Inmed`, Bank Transfer/Card → `Bank - Inmed`. | **B-12 deployed to TEST, pending functional smoke testing/prod promotion** | High |
...
| ID | Finding | Classification | Confidence |
|----|---------|---------------|------------|
| S6-F01 | **LOCALLY CORRECTED:** Multiple-tender double-counting is prevented. If a hospital/item appears in more than one active tender, Sales Invoice submit stops with a clear duplicate-tender message. | **B-04 deployed to TEST, pending functional smoke testing/prod promotion** | High |
| S6-F02 | **LOCALLY CORRECTED:** Sales Invoice submit now writes `tender_fulfillments` audit rows so the On Cancel script can reverse the exact tender quantities. | **B-05 deployed to TEST, pending functional smoke testing/prod promotion** | High |
...
| S6-F04 | **LOCALLY CORRECTED:** manual `frappe.db.commit()` was removed. Tender updates now stay inside the Sales Invoice submit transaction. | **B-13 deployed to TEST, pending functional smoke testing/prod promotion** | High |
...
| ID | Finding | Classification | Confidence |
|----|---------|---------------|------------|
| S9-F01 | **LOCALLY ADDED:** tender price is enforced before submit, so tender-covered invoice rows cannot bypass `Tender Agreement Item.tender_price`. | Tender price gap deployed to TEST, pending functional smoke testing/prod promotion | High |
...
| ID | Finding | Classification | Confidence |
|----|---------|---------------|------------|
| S8-F01 | **LOCALLY ADDED:** B-05 cancellation reversal is implemented from Sales Invoice tender fulfillment audit rows, so reversal does not depend on the tender still being Active. | **B-05 deployed to TEST, pending functional smoke testing/prod promotion** | High |
...
| ID | Finding | Classification | Confidence |
|----|---------|---------------|------------|
| S7-F01 | **LOCALLY CORRECTED:** manually Closed tenders are no longer reopened by date-based auto-status logic; S7 skips status auto-update only when `doc.status == "Closed"`. | **B-06 deployed to TEST, pending functional smoke testing/prod promotion** | High |
| S7-F02 | **LOCALLY CORRECTED:** only manual `Closed` status is preserved. `Draft` now participates in date-based status logic, so a future-dated tender can auto-transition to `Active` when the valid date range begins. | Draft status behavior deployed to TEST, pending functional smoke testing/prod promotion | High |
...
| ID | Finding | Classification | Confidence |
|----|---------|---------------|------------|
| X1-F01 | **LOCALLY CORRECTED:** Scheduler no longer uses the same task kind as dispatch-flow Finance debt tasks. `Debt Alert` and `Debt Collection` are separate, so a Director alert can no longer capture a Finance payment task. | **B-08 deployed to TEST, pending functional smoke testing/prod promotion** | High |
| X1-F02 | **LOCALLY CORRECTED:** Scheduler subject now uses `Debt Alert - {name}` while dispatch flow keeps `Debt Collection: {name}`. | **B-08 deployed to TEST, pending functional smoke testing/prod promotion** | High |
...
**Type:** Client Script — Form | **Enabled:** Yes | **Lines:** 77

#### Behavior Summary

On Dispatch Case form refresh, the UI script now separates payment/profit access from pricing access:

1. Payment/profit UI fields are shown only to `Ops - Accounting`, `Ops - Finance`, `Ops - Directors`, and `System Manager`.
2. Pricing UI fields (`unit_price`, `discount_pct`) are shown to `Ops - Order Creating` plus the financial roles.
3. Non-financial users have `payment_section`, `sales_invoice`, `prepaid_amount`, `prepaid_payment_entry`, `total_invoice_amount`, `total_paid_amount`, `outstanding_amount`, `profit`, and `advance_payments` hidden.
4. Non-pricing users have `unit_price` and `discount_pct` hidden in the `case_items` grid and child row detail view.

#### Findings

| ID | Finding | Classification | Confidence |
|----|---------|---------------|------------|
| C1-F01 | **LOCALLY CORRECTED:** client-side hiding is no longer the only planned protection. The B-10 deployment helper adds server-side permlevels/DocPerm rows, while the client script remains UI convenience. | **B-10 deployed to TEST, pending functional smoke testing/prod promotion** | High |
| C1-F02 | **LOCALLY CORRECTED:** `profit` and `advance_payments` are now included in the non-financial UI hide list. | **B-10 deployed to TEST, pending functional smoke testing/prod promotion** | High |
...
| C1-F04 | **Role split matches approved design.** Payment/profit fields are financial-only; price/discount fields are also available to `Ops - Order Creating` because they must enter prices during case creation. | Matches approved B-10 design | High |
| C1-F05 | **Server-side field protection is staged locally.** `deploy-b10-financial-field-permissions.ps1` sets Dispatch Case payment/profit fields to permlevel 1 and Dispatch Case Item price/discount fields to permlevel 2, with matching role permissions. | **B-10 deployed to TEST, pending functional smoke testing/prod promotion** | High |

#### B-10 Local Correction Direction

B-10 is locally corrected with Frappe field permissions/permlevels instead of relying only on client-side hiding.

Permlevel 1 Dispatch Case fields, restricted to `Ops - Accounting`, `Ops - Finance`, `Ops - Directors`, and `System Manager`:
- `sales_invoice`
- `prepaid_amount`
...
Permlevel 2 Dispatch Case Item fields, available to `Ops - Order Creating`, `Ops - Accounting`, `Ops - Finance`, `Ops - Directors`, and `System Manager`:
- `unit_price`
- `discount_pct`

The client script remains as UI convenience only; server-side access is handled by DocPerm/permlevel metadata after deployment.
...
|---|---------|-------------------|-----------------|--------|---------------|------------|
| 1 | Debt threshold alerts | Directors are alerted via separate `Debt Alert` tasks (Doc 16 §6.10B, `docs/manual/debt-alert.md`) | **Locally corrected:** scheduler creates/updates `Debt Alert` tasks for Directors | **Yes — pending functional smoke testing/prod promotion** | B-08 fixed locally; D-02 locally covered | High |
...
| 3 | Debt Collection task per customer | One active Finance payment task per customer (Doc 16 §6.10) | Dispatch flow enforces one active `Debt Collection`; scheduler no longer touches this task kind | **Yes — pending functional smoke testing/prod promotion** | B-08 fixed locally | High |
| 4 | Debt Collection assignment | Finance Team (Doc 16 §6.10) | **Locally corrected:** dispatch flow owns `Debt Collection`; scheduler assigns separate `Debt Alert` from policy | **Yes — pending functional smoke testing/prod promotion** | B-08 fixed locally | High |
| 5 | Open invoices table on Debt task | Shows all outstanding invoices on Finance `Debt Collection` tasks (Doc 16 §6.10) | **Locally corrected:** only dispatch-flow `Debt Collection` contains Open Invoices; scheduler `Debt Alert` intentionally does not | **Yes — pending functional smoke testing/prod promotion** | B-08 fixed locally | High |
| 6 | FIFO payment allocation | Oldest invoices first (req §6.6.2, manual Step 2) | **Locally corrected:** automatic FIFO now sorts by Sales Invoice posting date with invoice name as tie-breaker | **Yes — pending functional smoke testing/prod promotion** | B-11 fixed locally | High |
...
| 8 | Payment Entry auto-created | Auto-created with specified allocation (Doc 16 §6.10, §8) | **Locally corrected:** PE now receives Sales Invoice reference rows with preserved allocated amounts | **Yes — pending functional smoke testing/prod promotion** | B-01 fixed locally | High |
...
| 13 | Advance updates DC prepaid_amount | Amount noted on case (Doc 16 §6.12) | **Locally corrected:** linked-case advances append to `advance_payments`; `prepaid_amount` is recalculated from all rows | **Yes — pending functional smoke testing/prod promotion** | B-03 fixed locally | High |
...
| 16 | Debt Closure Approval | Locally documented in Doc 16 and `docs/manual/debt-closure-approval.md` | Task created on Debt Collection completion (S3) | **Yes — pending functional smoke testing/prod promotion** | D-01 locally covered | High |
| 17 | Profit calculation | Locally documented in `docs/manual/debt-closure-approval.md` | **Locally corrected:** calculated on Debt Closure Approval completion across all linked Open Invoices, with per-Dispatch Case profit writeback | **Yes — pending functional smoke testing/prod promotion** | B-07 fixed locally; D-01 locally covered | High |
...
| 18A | Tender price enforcement | Tender-covered invoice items must use `Tender Agreement Item.tender_price` | **Locally corrected:** Before Submit validation stops submit if invoice rate differs from tender price | **Yes — pending functional smoke testing/prod promotion** | Tender price validation added locally | High |
| 19 | Tender over-supply warning | System blocks submit if tender quantity exceeds remaining | **Locally corrected:** Before Submit validation stops submit with review message if invoice qty exceeds remaining tender qty | **Yes — pending functional smoke testing/prod promotion** | Control added; review with colleague | High |
| 20 | Tender cancellation reversal | Tender manual documents exact fulfillment reversal | **Locally corrected:** Sales Invoice cancellation reverses exact rows from `tender_fulfillments` audit table | **Yes — pending functional smoke testing/prod promotion** | B-05 fixed locally | High |
| 21 | Tender status auto-set | Auto-updates Draft/Active/Expired by dates while preserving manual Closed | **Locally corrected:** only Closed is preserved; Draft now auto-transitions by dates | **Yes — pending functional smoke testing/prod promotion** | B-06 plus Draft status confusion fixed locally | High |
| 22 | Financial field visibility | Non-financial roles cannot see payment/profit fields; pricing fields also available to Order Creating | **Locally corrected:** B-10 deployment helper stages server-side permlevels/DocPerm rows; client script now matches the approved role split | **Yes — pending functional smoke testing/prod promotion** | B-10 fixed locally | High |
...
| 24 | Scheduled debt check | Documented in Doc 16 §6.10B and `docs/manual/debt-alert.md` | Runs via scheduler, uses GL entries, creates/updates Debt Alert tasks | **Yes — pending functional smoke testing/prod promotion** | D-02 completed locally | High |
...
| **B-03** | Advance prepaid_amount overwrites — locally corrected | S4 lines 31-44 | Linked-case advances now append to `advance_payments`, `prepaid_amount` is recalculated from all rows, and `prepaid_payment_entry` remains latest-entry quick reference. | **Fixed locally, pending functional smoke testing/prod promotion** | Code: child-row append plus sum recalculation |
| **B-04** | Tender double-counts across multiple active tenders — locally corrected | S6 lines 18-70 | Duplicate active tender rows for the same hospital/item now stop Sales Invoice submit; exactly one match updates once; over-supply stops with review message. | **Fixed locally, pending functional smoke testing/prod promotion** | Code: duplicate/over-supply guards and single-match update |
| **B-05** | No tender reversal on invoice cancellation — locally corrected | S6 + S8 | Sales Invoice submit records exact tender fulfillment rows; Sales Invoice cancel subtracts those quantities from the same Tender Agreements. | **Fixed locally, pending functional smoke testing/prod promotion** | Code: `tender_fulfillments` audit rows plus After Cancel reversal script |

### Medium

| ID | Title | Script | Impact | Confidence | Evidence |
|----|-------|--------|--------|------------|----------|
| **B-06** | Tender "Closed" status overridden on save — locally corrected | S7 lines 14-22 | Manually Closed tenders now stay Closed on later saves; Draft/Active/Expired are date-driven. | **Fixed locally, pending functional smoke testing/prod promotion** | Code: `doc.status == "Closed"` skips auto-status |
| **B-07** | Profit calculation only used one invoice — locally corrected | S3 lines 85-116 | Multi-invoice Debt Closure Approval now sums profit across all unique Open Invoices and writes each linked Dispatch Case's own profit. | **Fixed locally, pending functional smoke testing/prod promotion** | Code: iterates `doc.open_invoices`, sums `total_profit`, writes `dispatch_case_profit` |
| **B-08** | Dual debt task creation paths with different assignees/data — locally corrected | S1 + X1 | Scheduler now creates/updates separate Director `Debt Alert` tasks; dispatch flow remains the only producer of Finance `Debt Collection` tasks with invoice detail. | **Fixed locally, pending functional smoke testing/prod promotion** | Code: S1 uses `DEBT_ALERT_KIND`; X1 remains `Debt Collection` |
| **B-10** | Financial visibility client-side only — locally corrected | C1 + deploy helper | Server-side protection is staged locally: payment/profit fields move to permlevel 1 for financial roles; price/discount fields move to permlevel 2 for Order Creating plus financial roles. Client script updated to hide `profit` and `advance_payments` too. | **Fixed locally, pending functional smoke testing/prod promotion** | `deploy-b10-financial-field-permissions.ps1` plus updated `Dispatch Case-Price Visibility.js` |
| **B-12** | `paid_to` hardcoded as "Cash - Inmed" — locally corrected | S2 + S4 | Debt Collection and advance Payment Entries now post to `Cash - Inmed` for Cash and `Bank - Inmed` for Bank Transfer/Card. | **Fixed locally, pending functional smoke testing/prod promotion** | Code: `paid_to_account` method mapping in both scripts |
| **B-13** | `frappe.db.commit()` inside loop in tender update — locally corrected | S6 | Manual commit removed; tender saves stay inside the Sales Invoice submit transaction. | **Fixed locally, pending functional smoke testing/prod promotion** | Code: no `frappe.db.commit()` in S6 |

### Low

| ID | Title | Script | Impact | Confidence | Evidence |
|----|-------|--------|--------|------------|----------|
| **B-09** | Hardcoded user emails in debt closure — locally corrected | S3 + Task-before-save-dispatch-gates | Approver assignment and completion permission now use `Debt Closure Approval` Task Access Policy; team/user changes no longer require script edits. | **Fixed locally, pending functional smoke testing/prod promotion** | Code: no `APPROVED_USERS` list; policy default team and allowed roles drive behavior |
...
| R-05 | **Completed locally:** added Sales Invoice tender fulfillment audit trail and `Sales-Invoice-on-cancel-tender-reversal.py` After Cancel reversal script. | B-05 | Done locally; deploy/test later |
| R-06 | **Completed locally:** S7 now preserves manually Closed tenders with `if doc.status == "Closed"`; Draft/Active/Expired are date-driven. | B-06 | Done locally; deploy/test later |
...
| R-13 | **Completed locally:** B-10 deployment helper stages Frappe permlevels/DocPerm rows for Dispatch Case payment/profit fields and Dispatch Case Item price/discount fields; `Ops - Order Creating` keeps price/discount access only. Client script retained as UI convenience. | B-10 | Done locally; deploy/test later |
...
| B-10 | `deploy/test/deploy/group-3-payments-debt-accounting/deploy-b10-financial-field-permissions.ps1`; `deploy/test/work/client/Dispatch Case-Price Visibility.js`; `docs/16-unified-dispatch-flow.md` §12.1 |
