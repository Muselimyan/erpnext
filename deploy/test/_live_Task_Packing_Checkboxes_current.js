...
function task_product_work_area_render_packing(frm, doc, rows, show_alert) {
    let html = `<div class="small text-muted" style="margin-bottom:8px">Source: <b>${frappe.utils.escape_html(doc.name)}</b> ÃÂ· Customer: <b>${frappe.utils.escape_html(doc.customer || "")}</b></div>`;
    html += `<style>
        .task-pack-mobile-dashboard { display: none; }
        @media (max-width: 767px) {
            .task-pack-desktop-table { display: none; }
            .task-pack-mobile-dashboard { display: block; }
            .task-pack-card { border: 1px solid var(--border-color, #d1d8dd); border-radius: 8px; padding: 10px; margin-bottom: 10px; background: var(--fg-color, #fff); }
            .task-pack-card-title { font-weight: 600; margin-bottom: 8px; line-height: 1.25; }
            .task-pack-card-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; align-items: end; }
            .task-pack-card-grid label { font-size: 12px; color: var(--text-muted, #6c7680); margin-bottom: 3px; display: block; }
            .task-pack-card-full { grid-column: 1 / -1; }
            .task-pack-card-value { border: 1px solid #d1d8dd; border-radius: 4px; padding: 5px 7px; min-height: 30px; background: #f8f8f8; font-size: 13px; }
            .task-pack-status { display: inline-block; padding: 3px 8px; border-radius: 999px; font-size: 12px; font-weight: 600; }
            .task-pack-status-ok { background: #e7f7ee; color: #0f7a3a; }
            .task-pack-status-open { background: #fff4e5; color: #946200; }
        }
    </style>`;
    html += `<div class="task-pack-desktop-table" style="overflow-x:auto"><table class="table table-bordered table-condensed"><thead><tr>
    ...
    let mobile_html = `<div class="task-pack-mobile-dashboard">`;
    rows.forEach(function(row, idx) {
        ...
        const mobile_checkbox_id = `pack_mobile_checkbox_${idx}`;
        const escaped_case = frappe.utils.escape_html(frm.doc.dispatch_case);
        const item_label = frappe.utils.escape_html(row.item_name || row.item_code || "");
        const batch = frappe.utils.escape_html(row.batch_no || "");
        const expiry = frappe.utils.escape_html(row.expiry_date || row.custom_expiry_date || "");
        const escaped_status = frappe.utils.escape_html(status);
        const escaped_warn = frappe.utils.escape_html(warn);
        html += `<tr>
            <td class="text-center"><input type="checkbox" id="${checkbox_id}" data-idx="${idx}" ${is_packed ? 'checked' : ''} onchange="task_product_work_area_toggle_packed(this, '${escaped_case}', ${idx})"></td>
            <td>${item_label}</td>
            ...
            <td>${batch}</td>
            <td>${expiry}</td>
            <td><span class="indicator ${is_packed ? 'green' : 'orange'}">${escaped_status}</span></td>
            <td>${escaped_warn}</td>
        </tr>`;
        mobile_html += `<div class="task-pack-card">
            <div class="task-pack-card-title">${item_label}</div>
            <div class="task-pack-card-grid">
                <div class="task-pack-card-full"><label><input type="checkbox" id="${mobile_checkbox_id}" data-idx="${idx}" ${is_packed ? 'checked' : ''} onchange="task_product_work_area_toggle_packed(this, '${escaped_case}', ${idx})"> Packed?</label></div>
                <div><label>Required</label><div class="task-pack-card-value text-right">${required}</div></div>
                <div><label>Scanned</label><div class="task-pack-card-value text-right">${scanned}</div></div>
                <div><label>Missing</label><div class="task-pack-card-value text-right">${remaining}</div></div>
                <div><label>Batch/LOT</label><div class="task-pack-card-value">${batch}</div></div>
                <div><label>Expiry</label><div class="task-pack-card-value">${expiry}</div></div>
                <div><label>Status</label><div><span class="task-pack-status ${is_packed ? 'task-pack-status-ok' : 'task-pack-status-open'}">${escaped_status}</span></div></div>
                ${warn ? `<div class="task-pack-card-full"><label>Warning / Problem</label><div class="task-pack-card-value">${escaped_warn}</div></div>` : ''}
            </div>
        </div>`;
    });
    html += `</tbody></table></div>`;
