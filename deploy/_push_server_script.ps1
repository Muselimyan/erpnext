...
$Uri = "$BaseUrl/api/resource/Server%20Script/task_list_filtered"
$escaped = $script.Replace('\', '\\').Replace('"', '\"').Replace("`r`n", '\n').Replace("`n", '\n').Replace("`t", '\t')
$Body = '{"script":"' + $escaped + '"}'
Write-Host "Pushing fixed server script ($($Body.Length) chars)..."
Invoke-RestMethod -Uri $Uri -Headers $Headers -Method Put -Body ([System.Text.Encoding]::UTF8.GetBytes($Body)) -TimeoutSec 30 | Out-Null
