# ==============================================================
# ERPNext Server Export  -  PROD (erpnext.am)
# Outputs:
#   deploy/prod/schema/*.json   one file per config / schema type
#   deploy/prod/data/*.csv      one file per master-data type (max 20 rows)
#   deploy/prod/export-summary.json
# Run with:  powershell -ExecutionPolicy Bypass -File deploy\prod\export.ps1
# For the test instance, use deploy\test\export.ps1 instead.
# See docs/infrastructure-test-vs-prod-environments.md for how to diff the
# two output trees once both have been exported.
# ==============================================================
...
Write-Host "Exporting from [Prod] $BaseUrl" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "============================================================"
Write-Host "Export complete [Prod] -> schema/ , data/"
