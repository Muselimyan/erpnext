# -*- coding: utf-8 -*-
"""
doc-resync-rebuild-warehouses-v2.py
1. Delete all existing H/D client warehouses
2. Create warehouses with names EXACTLY as in wh.txt:
   Hospital: "{code} — {hospital_name}"
   Doctor:   "{code} — {doctor_name}"  (hospital only used for parent lookup)
"""
...
# ── STEP 1a: Delete D warehouses (leaves first) ───────────────────────────────
...
for code, doctor_name, hosp_display in doctors:
    ...
    if not parent_full:
        ...
        continue
    # Name is EXACTLY "code — doctor_name — hospital_name" (em-dash between doctor and hospital)
    wh_name = f"{code} {EM} {doctor_name} {EM} {hosp_display}"
    wh_full = f"{wh_name} - {ABBR}"
    if frappe.db.exists("Warehouse", wh_full):
        log(f"  EXISTS: {wh_full!r}")
        continue
