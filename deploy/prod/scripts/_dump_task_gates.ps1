...
foreach ($n in @("Task-before-save-dispatch-gates", "Task-before-save-policy")) {
    Write-Host "`n=== $n ===" -ForegroundColor Cyan
    $r = Invoke-RestMethod -Uri "$BaseUrl/api/resource/Server%20Script/$(Enc $n)?fields=[`"script`"]" -Headers $Headers -Method Get -TimeoutSec 30
    $r.data.script
}
