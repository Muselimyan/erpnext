...
if ($Mode -eq "Deploy") {
    ...
    if ($itemNameField) {
        ...
        Write-Host "  ✓ Set item_name.in_list_view = 1" -ForegroundColor Green
    }
    ...
    if ($itemCodeField) {
        ...
        Write-Host "  ✓ Set item_code.in_list_view = 0 (hidden)" -ForegroundColor Green
    }
    ...
    try {
        ...
        Write-Host "  ✓ DocType updated successfully!" -ForegroundColor Green
    }
    catch {
        Write-Host "  ✗ Error updating DocType: $_" -ForegroundColor Red
        throw
    }
    ...
    if ($itemNameField.in_list_view -eq 1 -and $itemCodeField.in_list_view -eq 0) {
        Write-Host "`n✓ Already configured correctly!" -ForegroundColor Green
    } else {
        Write-Host "`n⚠ Needs configuration. Run with -Mode Deploy" -ForegroundColor Yellow
    }
}
